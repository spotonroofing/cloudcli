...
- [ ] Numbers sit on the line: wherever a mono or tabular number renders inline with text (the "Thinking 3m18.9s" timing, token counts, line counts, durations, the context ring label, anywhere the sweep finds), the number's vertical center matches the adjacent text's vertical center. Fix the cause once (the mono font's metrics and line-height or vertical-align in the shared number/mono class), not per site, and apply the shared class app-wide.
...
## Phase 6 — Worker pane: status badge, live updates, phase navigator redesign

Goal: the worker pane is live and legible: no status badge, real-time transcript for dispatched runs, and a phase navigator that shows every phase with a task drawer, per-task status icons, and a per-phase counter. Files: the worker pane header and phase navigator components, the worker pane's realtime wiring (src/contexts/WebSocketContext.tsx, the sessions watcher path in server/modules/providers/services), server/modules/watchdog (manifest handling, chain events), the dispatch chain runner, the manifest type (ChainManifestEntry). Dependencies: none.

- [ ] Remove the status badge from the worker pane's top row entirely, in every state (running, finished, any other); state is carried by the phase navigator and run switcher.
- [ ] Dispatched runs render live: a worker pane following a dispatched phase shows tool calls, thinking, and text as they happen, with no refresh. Reproduce with the running or a stub chain, find why the pane fell out of sync (the ui11 phase 1 pane showed nothing while the run was active), and fix the cause; add a regression check.
- [ ] Phase navigator redesign: all phases are listed at once (the navigator is a scrollable list, not a single-phase strip), each phase row is a drawer that expands to show its tasks; the active phase's drawer is open by default, others collapsed, all toggleable. Each phase row carries a done/total counter on its right (total from the manifest; done from that phase's items checked off in the run's punch list file, matched by order; the manifest gains an optional punch list path and per-phase heading anchor so counts are exact; the watchdog re-reads the file on each chain event and after each commit). The phase ring advances with the counter.
- [ ] Task rows inside a drawer show a status icon instead of a bullet: a check for done, a working indicator for the first unchecked task of the active phase, an idle marker for untouched tasks. Runs without a manifest show phase numbers only, no counters or drawers.

Done check: on dev, a stub chain with a manifest and a test punch list (stub claude per lesson chain-runner-fully-stubbable-via-env): the status badge is absent from the header DOM in the running and finished states; the pane shows the stub's output live without reload; all phases list at once, the active drawer is open, others collapsed and toggleable; the counter reads 0/3, then 2/3 after a stub commit checks two boxes, with check/working/idle icons in the right rows. Holds at a phone viewport. Fresh-context subagent verification. Commit.
