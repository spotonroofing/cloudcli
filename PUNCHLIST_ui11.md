...
- [ ] `dispatch append` carries tasks: it accepts a manifest fragment (or reads a `<!-- tasks: ... -->` list from the top of each appended prompt file) so appended phases announce their task rows and counters like dispatched ones; today they announce with empty task lists (scripts/macos/dispatch, the append branch).
