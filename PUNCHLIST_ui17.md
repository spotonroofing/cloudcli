...
Done check: on dev with agent-browser on a session with a long transcript: scroll up 800px, inject three new rows through the stub, scrollTop unchanged; the footer shows the split figures against a fixture with known usage; the meter for a `claude-opus-5` session with 204,614 cache-read tokens reads 20 percent of 1 million; tests pass. Commit.
