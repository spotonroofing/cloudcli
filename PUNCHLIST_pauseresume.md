...
- [x] Breakdown for Codex: the popover shows what the rollout knows for the last turn, labeled plainly: input from cache, fresh input, output, reasoning, and the window; no Claude-only categories, no invented split; when no `token_count` event exists yet it says "no reading yet".
- [x] Live: the figure and breakdown update as new `token_count` events land in the rollout, pushed over the existing WebSocket from the same sessions-watcher hook that feeds the ChatGPT usage meters; no refresh needed; nothing polls while nothing is watching.
