...
- [x] The left pane is origin-aware: it renders the Planner header only for sessions whose origin is planner or null (Willem's chats). If a worker-origin session (direct, dispatch, external) becomes the selected session, the header says Worker honestly. A worker session never renders under a Planner label.
- [x] Planner boot dedupe: one boot click creates exactly one session row. The DB shows three planner boots created in the same second on 08-19; find and guard the multi-fire path. (Forensics: those same-second rows were the 08-18 27-project fleet boot — one boot per project, different projects, no same-project dupes exist in either DB. The real residual window — a second New Session trigger while a boot's session-creation POST is in flight — is now guarded in useChatComposerState.)
- [x] A planner handoff to the worker starts a fresh worker session automatically; the prior run stays in the switcher, pushed back, never deleted. Willem typing into an existing worker chat continues that chat — no new session. (Dispatch runs always create fresh sessions; the switcher keeps the 10 newest and deletes nothing; auto-follow now also holds while the composer is focused so a landing run cannot swap the session mid-typing.)
- [x] Connection fail-safes: if a pane's rendered stream is not the session it claims, or the socket subscription drops, show a visible broken/disconnected state in that pane's header. What the planner hands the worker is always visible in the worker pane, never hidden.
...
- [x] Accent: orange becomes silver-blue, leaning silver, as CSS variables in both themes; sweep the app for orange remnants (search hex values and Tailwind classes). DESIGN.md updated with the new accent tokens. (Light `211 20% 45%`, dark `211 22% 64%`; ring/nav tokens follow. Orange remnants: denied badge and warning modal → amber, git push button → primary, skills project badge → violet. Kept: file-type icon colors and the commit-graph categorical palette — data colors, not accent.)
- [x] Thinking indicator: fix the lag (it visibly trails). The loader becomes beUI's ASCII Braille mission loader rotated to horizontal, sitting left of the word "thinking". (beUI has no "mission" loader — voice-to-text drift; built the `ascii-braille` frame set as a horizontal 3-cell wave, `data-slot="braille-loader"`, left of a shimmered "Thinking". The trailing was real: the transcript never followed growing streamed content — fixed by the beUI MessageScroller's ResizeObserver follow engine.)
- [x] Streaming response: steal beUI's streaming response component, with playback roughly 15% faster than its default. (Vendored wrapper + reveal at 126 chars/s = 110 × 1.15. Required enabling `includePartialMessages` in the Claude SDK runtime — the provider never emitted stream deltas before, replies landed whole.)
- [x] Message enter/load animation redone with beUI patterns — no vanish-then-reappear on load. (Live rows play the beUI MESSAGE_POP_UP spring keyed off a per-session mount epoch; history renders statically; mutation-observer check on session switch showed rows mount once, no churn.)
- [x] Message timestamps right-aligned, visible only on hover with a fade-in, both panes. (Both panes share MessageComponent; opacity-0 → group-hover fade, always visible on touch.)
- [x] Code block, tool result, and message scroller components stolen from beUI and rethemed. (Code block chrome on the existing highlighter; tool rows — one-line, collapsible, Bash, subagent, group, error — moved from the left-rule law to the beUI tool-result treatment with AgentDisclosure; MessageScroller vendored without its optional preview-rail navigation, which has no honest home next to the export-menu edge.)
- [x] Font audit on the chat surface: Willem reported it looked off; confirm the intended family, size, and smoothing actually render, and fix what does not. (Verified in computed styles on dev: Schibsted Grotesk Variable loaded and applied to body and message content, 14px, antialiased both axes — nothing broken found; the "off" look was plausibly the pre-round accent/tool chrome. One real rendering bug fixed: two-digit ordered-list markers clipped to one digit by `pl-5` + `contain: paint`.)

Found during verification (not a listed item): the boot-prologue filter keyed on session origin, so a chat started by typing a first message (origin planner/direct, never booted) had that first turn — user bubble and tool rows — permanently hidden. Fixed honestly: sessions now carry a `booted` stamp set only when the first message is an auto-sent boot prompt (migration backfills existing planner/direct rows to preserve their current behavior), and the filter keys on it end to end (DB, serializers, worker-runs, deep-link resolution, pane latch for live boots). Verified both directions on dev.
...
- [x] Character counter on the composer. (`data-slot="char-counter"`, NumberTicker rolling digits, visible whenever input is non-empty; live-verified per keystroke on dev.)
- [x] Paste-as-file: a paste above roughly 2,000 characters collapses into a pasted-file attachment chip, Claude-desktop style, and is delivered to the session as a file attachment through the existing attachment path. (Threshold 2,000 in `useChatComposerState.handlePaste`; "Pasted text.txt" chip → `/api/assets/files` upload → `<files_input>`; verified end to end on dev including the stored asset on disk.)
- [x] beUI controls stolen and rethemed wherever the app has an equivalent: tabs, switch (this app's box radius, not pill), select animation, combobox, radio group, badges, animated context menu, function swap, stack, text and number animations. Butter and mission are available as loading-overlay treatments where an overlay already exists. Anything with no honest home is skipped and named in the run summary, never forced in. (Adopted: tabs — settings sidebar, git panel, mobile workspace rail; switch — SettingsToggle + DarkModeToggle, box radius; select — appearance tab + language selector; radio — AskUserQuestion single-select rows; badges — animated Badge skin + status pills; context menu — file-tree clip-path unfold; function swap = Action Swap on composer submit + mic icons; number/text — NumberTicker on counter/elapsed/context value, SwapText on model pill; loaders — dot-matrix/dither on all true loading overlays. Skipped, no honest home: combobox (only bespoke menus exist), stack (one single toast, no stacked notifications; toast rethemed to tokens instead), "butter"/"mission" (no such beUI loaders exist — voice-to-text drift, same as phase 3's "mission loader").)
- [x] Tool-permission/approval prompt restyled from beUI's approval component. (`ToolApproval.tsx` from beui.dev tool-approval; all three decisions, batch remember, and the AskUserQuestion custom-panel path preserved. Not live-triggerable on dev — sessions are hardwired skip-permissions — verified by code review and typecheck.)
- [x] Worker to-do display: where task lists render in worker runs, use beUI's to-do list component. (Vendored beUI `TodoList` behind `TodoListContent`/ToolRenderer, statuses mapped; `Queue.tsx` retired. Not live-triggerable — TodoWrite is disabled in this instance's sessions — wiring verified by fresh-context review.)
- [x] Citations component wherever the transcript renders references or links. (Assistant messages with external links get one deduped "Sources" strip after the body; `extractExternalLinks` unit-tested incl. image-embed exclusion; live-verified on dev with a 2-source strip.)
...
- [x] AI sidebar: steal beUI's AI sidebar as the base, keep the current top-of-sidebar layout and content exactly, retheme to this app's tokens; fold in the bounce-sidebar behavior. (Adopted the ai-sidebar row anatomy across the desktop Projects tree — borderless `min-h-9 rounded-lg` rows, icon tile, overflow-aware `MarqueeLabel` vendored verbatim, hover-revealed actions, spring chevron, depth-padding indent with an eased unfold — on the existing data plumbing; a wholesale swap to beUI's tree engine would have rebuilt sessions pagination/rename/move for no visible gain. Top bar (segmented control, icon cluster, search) untouched. Bounce-sidebar folded in as `BounceIndicator`: the donor's curved-spring dot physics arcing to the selected session row via `data-bounce-key`.)
- [x] Project icons: copy existing icons from spoton-core assets for the SpotOn projects present in CloudCLI; scan each project repo root for `app-icon.svg` / `icon.svg` / `icon.png` and prefer it when present; one good beUI-styled default icon for everything else. Icons sit left of the project name. (Six spoton-core icons committed into `public/project-icons/` — sign, cam, book, payroll, stats, design(→spoton-core) — keyed by repo basename, no live spoton-core dependency; repo-root convention wins when present; served as `iconDataUrl` on the project payload; default is the beUI ai-sidebar's own Folder/FolderOpen tile. Live-verified: SPOTON CAM renders the cam SVG, CLOUD CLI the default.)
- [x] Activity shimmer: vendor border-beam (rotate mode, mono colorway, rethemed neutral — silver, not SpotOn blue). Chat rows with a running turn shimmer; a project row shimmers while any of its sessions runs; once the project is expanded its own row's shimmer fades out and only the running chat rows shimmer. Every appearance and disappearance is a fade, no hard cutoff. Keep it subtle: opacity and filter, breathing not flashing. (Vendored slim from the spoton-core wrapper over npm `border-beam`: rotate+mono only, ink `--beam-ink` silver per theme, ambient 0.4; `useBeamPresence` keeps beams mounted through the engine fade. DOM-verified live on dev: running chat row `data-active`, collapsed project row `data-active` while its session ran, expand sampled `data-fading` for ~450ms then unmount, idle DOM has zero beam elements.)
- [x] Active-session counters at the top of the sidebar: a planner count and a worker count, visually distinct from each other, with a soft shimmer while nonzero, placed without disturbing the existing top-bar layout. The collapsed sidebar rail shows planner icon + count and worker icon + count with the same shimmer. (`ActivityCounter` pills in a new slim row under the search bar — planner = Compass/primary, worker = Hammer/emerald, the pane-header icons; NumberTicker digits, text-shimmer while nonzero. Rail: `RailCounter` icon-over-count pair. Counts derive from the run registry enriched with origin+projectId (`/sessions/running`), unioned with live websocket activity; planner = origin planner/null, worker = direct/dispatch/external — cross-checked live against the registry payload, 1/0 both sides mid-run, 0/0 idle.)
- [x] Projects-tab chat names drop to the same font size the Chats tab uses. (`text-sm` → `text-[13px] leading-4` on both desktop and mobile session rows; computed 13px asserted on dev.)
- [x] Short summaries: implement the Haiku-at-creation label (decisions block above); show it in the worker run switcher and the Projects-tab chat lists in place of long first-message text. (`session-label.service.ts`: one fire-and-forget `claude -p --model claude-haiku-4-5-20251001 --no-session-persistence` per session at creation — and at the boot-placeholder seam for first typed messages — sanitized, written to `custom_name` only if the fallback title is still in place, broadcast via `session_upserted`. The switcher and chat lists read `custom_name`, so both show it with no further wiring; truncated-first-message stays the fallback. Live sqlite assert: label stored ~12s after creation, sidebar row updated in place. Found+fixed live: without `--no-session-persistence` each label call left a phantom external session titled with the label prompt.)
- [x] TV rail: if a horizontal summary strip has an honest home for planner chat summaries, steal beUI's TV rail for it; if not, skip and name it in the run summary. (Skipped: beui.dev has no TV-rail component — the live registry's closest names are `preview-rail` (a vertical nav rail, already evaluated and skipped in phase 3) and `marquee`; same voice-to-text drift as phase 3's "mission loader". Independently, the sidebar has no honest home for a horizontal planner-summary strip without rearranging the top layout this phase is required to preserve.)
