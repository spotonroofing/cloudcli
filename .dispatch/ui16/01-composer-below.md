...
<!-- model: gpt-5.6-sol -->
<!-- effort: high -->
<!-- name: Composer controls below -->
<!-- tasks: Counter bottom left under the bar | Usage, switcher, history, handoff right | Box holds plus, voice, send only -->
Execute Job 1 of PUNCHLIST_ui16.md in this repo (the composer controls move under the prompt bar). The goal and items are that job's section and checkboxes in PUNCHLIST_ui16.md; read only that job's section plus the whole-file rules block at the top (including the run rules), the recent git log, and the files the job names; trust repo state over this prompt when they disagree, and check each item off in the file the moment it is verified. Done when the job's done check passes against the dev instance.
