...
<!-- effort: high -->
<!-- supersedes: ui17/3 -->
<!-- name: Heap under budget -->
<!-- tasks: Remaining holder found and cut | Memory budget passes twice -->
Execute the one remaining unchecked item of Job 2 of PUNCHLIST_ui17.md in this repo (the memory scenario's heap growth still exceeds its budget). The goal and items are that job's section and checkboxes in PUNCHLIST_ui17.md; read only that job's section plus the whole-file rules block at the top (including the run rules), the recent git log, and the files the item names; items already checked stay done; trust repo state over this prompt when they disagree, and check the item off in the file the moment it is verified. Done when the job's done check for that item passes against the dev instance.
