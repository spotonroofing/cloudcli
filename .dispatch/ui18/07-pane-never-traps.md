...
<!-- model: claude-opus-5 -->
<!-- effort: high -->
<!-- name: Pane never traps, for real -->
<!-- tasks: Reproduce with the Codex session, numbers | Indicator flush above the composer, no lock | Tests on the exact state | Verifier reproduces first -->
Execute Job 6 of PUNCHLIST_ui18.md in this repo (the pane never traps its reader, second attempt with the real reproduction). The goal and items are that job's section and checkboxes in PUNCHLIST_ui18.md; read only that job's section plus the whole-file rules block at the top (including the run rules), the recent git log, and the files the job names; trust repo state over this prompt when they disagree, and check each item off in the file the moment it is verified. Done when the job's done check passes against the dev instance.
...
Record lessons in /Users/spoton-worker/Projects/spoton-worker/planner/command-center/lessons/ (this is the memory repo, not the repo you are working in): one lesson per file, a one-line summary as the first line; corrections and confirmed approaches alike, including why they mattered; never save what the repo or PROJECT.md already records; update an existing note rather than duplicating.
