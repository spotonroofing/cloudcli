...
<!-- effort: high -->
<!-- supersedes: ui15r4/4 -->
<!-- name: Memory popout on phone -->
<!-- tasks: Popout fits the phone viewport | Prompt bar reachable and keyboard-safe | Desktop unchanged -->
Execute the one remaining unchecked item of Job 4 of PUNCHLIST_ui15.md in this repo (the memory popout is clipped on phone). The goal and items are that job's section and checkboxes in PUNCHLIST_ui15.md; read only that job's section plus the whole-file rules block at the top (including the rerun rules), the recent git log, and the files the item names; items already checked stay done; trust repo state over this prompt when they disagree, and check the item off in the file as verified. Done when the job's done check passes against the dev instance at the phone viewport.
