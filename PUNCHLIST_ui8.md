...
- [x] The two-pane (single-project) planner/worker divider is draggable with the same mechanics the stacked view already has; default split is 50/50; the current larger-planner anchoring is gone. (MainContent's fixed `w-[44%]` worker is now weight-based flex driven by `workspace.split` — one shared, persisted fraction for the two-pane view and rows mode — with the workspace's slim divider bar; DEFAULT_SPLIT 0.56 → 0.5. Fresh profile measured 573.5/573.5px; drag persisted 0.371 across reload; clamps hold at 0.25/0.75.)
- [x] Drag-to-combine: drag a project row from the sidebar into the open view; drop zones with snap guides — left/right edge produces the column layout, bottom edge produces the stacked layout. The top-left stacked-rows toggle button on sidebar rows is removed; whatever layout switching remains lives somewhere sensible and obvious (worker's call, not a lit icon in the row). (Desktop sidebar rows are HTML5-draggable; WorkspaceView is the drop surface — left/right 20% bands → columns at start/end, bottom 25% → rows appended, zone wash + the standard 2px snap guide mid-drag, `openProjectAt` sets order+mode. Rows2 sidebar toggle deleted; layout switching stays as the workspace row-header toggle. All three zones verified on dev; a foreign-type drag or self-drop of the sole project is ignored.)
- [x] Closing an open project row: hover over that project in the sidebar reveals a close-row control; the in-view close moves out of the planner header to a clearer per-row spot. (Sidebar rows of open workspace projects get a hover-revealed X (`data-workspace-close`) wired to the same close-with-primary-handoff path; the planner-header X is gone from workspace rows — the row's close is the worker header's trailing X at the row's top-right, labeled "Close {name} row". Hover reveal, both close paths, and primary handoff verified on dev.)
...
- [x] Panel modeled on Willem's reference for data and interaction, restyled to this app: one row per account — slot number, email, active marker, and 5h / 7d / Fable usage bars with percentage and reset time; hovering a row reveals its actions (use, disable/enable, reorder up/down) the way the reference does; an add-account row at the bottom. Our tokens and radius, not the reference's look; no pills. (AccountsPanel on the shared Dialog: rounded-sm bar tracks, primary/amber/destructive fills at <70/70/90, reset countdowns from cswap, hover-revealed row actions fenced with md:, add-account expands to a masked token input + optional email.)
- [x] Mobile: the panel works tap-first — actions visible or behind a tap, 44px targets, safe-area respected. (390px pass: actions visible with no hover, 28px buttons carry touch-hit for the 44px coarse-pointer hit area, panel fits the viewport, inputs text-base below md.)
- [x] Switching accounts takes effect for new sessions; the active marker and usage refresh after a switch. (Switch re-runs the dev launcher's Keychain mirror server-side — after a UI switch the config-dir-scoped item and .credentials.json hash-match the default item, so new sessions read the new account; the panel refetches after every action and the footer label follows.)
