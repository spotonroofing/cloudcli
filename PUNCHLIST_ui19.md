...
- [x] The Claude model catalog is sourced from the installed CLI at runtime rather than a hardcoded array: find what the CLI or its SDK exposes (a models listing, the SDK's known-model data, or a cheap probe path) and read it at server start and on a settings refresh, falling back to the current static list only when the source is unavailable; the summary states exactly which source was used and why. After a `claude update` introduces a new model, the switcher shows it after a server restart or refresh with no code change.
- [x] `claude-fable-5-1` (label Claude Fable 5.1) replaces `claude-fable-5` as the Claude default everywhere a default lives: the provider `DEFAULT`, the new planner and worker session defaults in `settings.service.ts`, the client-side fallback in `useChatProviderState.ts`; Fable 5 stays selectable in the list; the context-window map carries 1,000,000 for `claude-fable-5-1`.
- [x] Sessions already running keep their model; only new sessions pick up the new default; the account switcher's Fable meter keeps working (its label reads the Fable family, not one id).
