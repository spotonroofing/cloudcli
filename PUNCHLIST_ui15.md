...
- [ ] The Handoff button returns as a visible control in the bottom row: leftmost of the group sitting left of the model switcher (handoff, then prompt history, then the switcher); the plus-menu entry may stay or go, worker's call, but the one-click button exists again.
