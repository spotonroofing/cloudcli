...
- [ ] Middle-row shift found and fixed with evidence: measure the nine cells' bounding boxes per row and column on dev at device pixel ratio 1 and 2; the likely cause is fractional geometry (4px cells with a 1.5px gap snap rows to different pixels); land whole-pixel geometry so every row shares one y and every column one x, for round and square cells alike.
- [ ] Stray patterns found and fixed with evidence: over a long planner turn record which patterns render and when; candidates are the rotation index jumping when the server's status text replaces the word, the elapsed counter restarting on an activity change, or an exiting indicator overlapping an entering one. At any moment exactly one loader is in the DOM, playing exactly one catalog pattern.
