...
- Rerun rules (ui15r2r, 2026-08-27): jobs run on Codex GPT-5.6 Sol with the runner's Luna verify stage; do not spawn subagents and skip any "fresh-context subagent" wording in a done check, the runner's verify stage is that check. Items marked done in the codex round stay checked; do not redo them. On unrecoverable failure stop and state what blocks.
...
- [ ] Worker pane follow stability: with pipelined verification the pane flips between the build session and the verify session, loading and unloading repeatedly; the pane follows one session deliberately (the build session by default, verify reachable through the jobs view's verify row) and never auto-switches mid-view; reproduce with a stub pipelined chain and add a regression check.
