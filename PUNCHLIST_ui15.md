...
- [ ] Drawer order inside a job: its tasks, then the "Verified" row (with its time) directly under the last task, then the engine and model line, then the commit line, then the job's total time at the very bottom.
- [ ] Task rows: each task carries the time it took (from the task's start to its check-off); the time shows only on hover, right-aligned in the same muted mono as the verified time. Hovering a task highlights its text, removes the strikethrough, and scrolls the label horizontally (a marquee, ramped, once through then back) so a truncated label can be read in full; leaving the row restores the truncated struck-through state. Touch devices get the same on tap.
- [ ] The design area file documents the row anatomy, the drawer order and the hover reveal.
