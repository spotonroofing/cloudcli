...
- [ ] The footer lives inside the job's drawer, directly after its last task row: the commit subject with the short hash plus the job's total duration (from the job's start and end events), monochromatic, marquee if long. A running job shows the footer too, with a live elapsed counter ticking from its start event (lining tabular digits, the "1m 50s" format) that settles into the final duration at completion; idle jobs show none.
