 ...
 * this state only shapes the UI once a second project is opened. The
 * planner/worker split inside each row lives in the per-project window store
 * (`useProjectWindows`, ui13 job 10), not here.
 */
export function useWorkspace({
  ...
  useEffect(() => {
    try {
      writeSetting(STORAGE_KEY, JSON.stringify({ order, mode, weights }));
    } catch {
  ...
  const openProjectAt = useCallback((projectId: string, index: number, nextMode: WorkspaceMode) => {
    ...
    // Deterministic initial split (ui13 job 10): combining projects lays the
    // grid out evenly instead of inheriting stale drag weights.
    setWeights({});
  }, []);
