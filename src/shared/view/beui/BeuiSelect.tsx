...
// beUI Select (beui.dev/components/motion/select), vendored with donor
// spacing/sizing/motion verbatim. Retheme: radius on the app token
// (rounded-lg = 8px, so the gooey radius keyframes run 8 instead of 12),
// trigger on the app input tokens (border-input bg-card, ring through
// `ring`), panel on the popover spec (border-border bg-popover shadow-lg),
// selection tick through `primary`.
...
export function Select({
  ...
  return (
    <SelectContext.Provider value={ctx}>
      <div ref={rootRef} data-slot="beui-select" className={cn('relative', className)}>
        {children}
      </div>
...
export function SelectTrigger({ className, children }: SelectTriggerProps) {
  ...
  return (
    <motion.button
      ...
      className={cn(
        'relative z-10 flex w-full touch-manipulation items-center justify-between gap-2 rounded-lg border border-input bg-card px-3 py-2 text-sm text-foreground outline-none transition-colors',
...
export function SelectValue({ placeholder, className }: SelectValueProps) {
  ...
  return (
    <span
      className={cn('truncate', label ? 'text-foreground' : 'text-muted-foreground', className)}
    >
...
export interface SelectContentProps {
  className?: string;
  /** Extra classes on the inner list (e.g. a max-height + internal scroll). */
  listClassName?: string;
  children: ReactNode;
}

export function SelectContent({ className, listClassName, children }: SelectContentProps) {
  ...
  // React 18: `inert` isn't a typed boolean prop yet — set the bare attribute
  // while closed so the mounted-but-hidden items can't take focus.
  ...
  return (
    <motion.div
      ...
      className={cn(
        'absolute left-0 right-0 z-20 rounded-lg border border-border bg-popover shadow-lg',
  ...
  return (
    <motion.li variants={ctx.reduce ? undefined : ITEM_VARIANTS} className="list-none">
      <button
        ...
        className={cn(
          'flex w-full touch-manipulation items-center justify-between gap-2 rounded-md px-2.5 py-1.5 text-left text-sm outline-none transition-colors',
          selected
