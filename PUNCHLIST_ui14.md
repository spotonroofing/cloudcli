...
- [ ] The project row's working border beam fires for worker activity too: it lights whenever that project's planner OR any of its worker sessions (dispatched, direct, maintenance) is mid-turn, driven by the same live session-state source the counter drawers use, and clears when all are idle; today it reacts only to the planner.
- [ ] Planner and worker top bars are the exact same height: one shared header component (or one shared height token and padding set) drives both, so no per-pane control can change the bar's height; the worker bar is currently taller. Strict check: computed heights equal to the pixel at desktop and phone widths, with and without the jobs toggle, plus button, and session title present.
- [ ] One error row for the whole app: session-level errors (limit messages, provider errors, anything that today renders as the red "Error" block or a bare text line) render as a single error row matching the tool-row anatomy (leading red icon, short label, one-line summary, chevron), collapsed by default and expanding to the full error text; themed like everything else, no bespoke red box. The "No response requested" line never renders as part of an error.
- [ ] Project list order is stable: rows never reorder because a running session made a tool call or streamed; with two projects active the list sits still. Pick a deliberate order (last opened by Willem) that changes only on his own actions; regression check with two streaming stubs.
- [ ] Project names stop truncating early: "SPOTON GATEW"/"COMMAND CEN" cut off while the row has free space; the name uses the available width (count and chevron take fixed slots) and the hover marquee covers genuine overflow.
- [ ] Chat navigation never disturbs the layout: expanding a project's chat list is a pure dropdown (no pane or workspace change); clicking a chat of an already-open project swaps that project's planner pane to that chat in place, nothing else moves; a project not yet open opens only when a chat is clicked, not on row or chevron click. Regression: with a two-project column view, expand lists and switch chats on both sides; the layout never resets.
- [ ] Job history rows older than 24 hours show a compact date next to the duration (e.g. "Aug 25"), lining digits, muted meta style.
