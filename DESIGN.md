...
App chrome
- Sidebar: `src/components/sidebar/`; main layout: `src/components/main-content/`, `src/components/app/`. The main chat pane and the worker pane each carry the same slim header bar (`border-b border-border/60 bg-muted/30 px-3 py-1.5`, muted icon + `text-xs font-medium` label): "Planner" with the open session's title in `MainContent.tsx`, "Worker" with the run switcher in `WorkerPane.tsx`. New pane-level headers extend this bar, never a parallel style. `SidebarHeader.tsx` is one shared header (equal-width segmented control in the top button bar + ghost icon cluster + search; mobile adds the Running segment and create-project button, desktop the collapse button); `SidebarFooter.tsx` is the single bottom-pinned Settings row on both form factors.
