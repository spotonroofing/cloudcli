...
Start here before UI work. Read only the areas your files touch. Reuse the closest existing element documented there; a new variant extends the existing pattern, never a parallel style. When implementation changes a documented pattern, update its owning area file in the same change.

## Areas

- [Tokens](design/tokens.md) — brand, typography, color, radius, spacing, shared primitives, and control foundations.
- [Sidebar](design/sidebar.md) — project and chat rows, activity, footer drawers, accounts, counters, and memory.
