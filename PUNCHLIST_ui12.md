...
- [x] Find every path where a client-local (localStorage or in-memory) queued message can still reach send: boot flush of pre-ui11 localStorage keys, reconnect replay, visibility-change flush, anything else the sweep finds. Remove the legacy flush entirely: on boot a client purges pre-ui11 local queue keys without sending them, and reconciles only against the server queue.
- [x] A queued message consumed on one device (sent, edited, cleared, or steered into a turn) is authoritative on the server; every other device applies the change live and a device coming back online adopts server state, never pushes its stale copy.
- [x] Walk the full lifecycle matrix and fix what fails: queue then close tab, queue then shut down and return, queue on A edit on B, queue while a turn runs then reconnect mid-turn, queue then session switch. List each case and its result in the summary.
...
- [x] Force lining figures app-wide for numeric UI: `font-variant-numeric: lining-nums` (plus `tabular-nums` where columns align: counters, timers, token counts) on the shared number style, and verify the loaded font honors it (computed style plus a rendered-glyph bounding check); if the font family itself lacks lining figures in the weight used, swap the numeric style to a weight or family that has them. Sweep the app for digit sites not on the shared style (the composer character counter is one) and put them on it. *(Verified: body carries lining-nums + a universal `--tw-numeric-figure` default so Tailwind's tabular-nums keeps it; counter and timer compute `lining-nums tabular-nums`. Font-level check: Schibsted's GSUB has no onum/lnum — its only figures ARE lining (canvas ink tops uniform within 0.48px at 40px), so no family swap needed; the visible ride-high came from NumberTicker's composited digit columns pixel-snapping fractional 1.1em steps, fixed by snapping the step to whole device pixels. Counter moved onto font-mono.)*
- [x] Thinking/turning duration format: consult beautifului's transcript components in the GitHub mirror TurboKach/ai-native-react-components (beautifului.dev itself is unreachable from this network, see the lesson) for their duration convention; adopt it exactly (unit spacing like "1m 50s" versus "1m50s", casing, tabular digits). If the mirror shows no duration component, use "1m 50s" with a thin space and note that in the summary. The counter's digits sit on the text baseline per the lining fix. *(Verified: mirror's loading-state.tsx formats `${s.toFixed(1)}s` under a minute, `` `${m}m ${s.toFixed(1)}s` `` above — REGULAR ASCII space, not thin space; ActivityIndicator already matched exactly, confirmed rendering "1m 21.1s" on dev with digit boxes within 0.002px.)*
...
- [x] Web search and web fetch tool calls (any provider's search/fetch tool names the renderer sees) render as a Research row: a "Research" label with the query or domain as the description line, collapsed by default, expanding to the sources (title and URL per result where the tool result carries them, clickable, opening in a new tab). Sources come only from actual tool results; nothing is synthesized. (`ResearchDisplay.tsx`; the renderer sees WebSearch/WebFetch from every provider — codex normalizes `web_search` to WebSearch server-side.)
- [x] The Running and Error tag chips on tool rows are removed: a running row shows the ramped spinner as its leading icon, an errored row shows a red status icon and its first error line in the description (semantic colors per phase 4); the chevron and row layout stay per ui11's alignment work. (`ToolRowStatusIcon` replaces `ToolStatusBadge` everywhere; Bash's `$` slot swaps to the icon, so no layout shift.)
- [x] Find why the ring spinner does not animate anywhere in the app (Willem sees static rings in Brave; check whether the CSS animation is being stripped, an SVG animation is unsupported, or a reduced-motion query misfires) and fix it at the shared component so every spinner in the app rotates; verify in a Chromium profile with default settings. (Root cause was the motion rotate under `initial={false}` that never played — fixed at the shared component in job 8 via the CSS `spinner-ramp` animation; verified here in a default Chromium profile: computed animation-play-state `running`, transform changes across frames, on both the TodoList ring and the new tool-row spinner.)
...
- [x] Initial load fetches enough pages to at least fill the viewport (keep fetching until the container is scrollable or the transcript is exhausted), then pins to the bottom with the newest message above the composer at the standard padding; the same holds on refresh, session switch, and a worker pane following a live run. *(Verified: `resolveTranscriptFillAction` + a fill effect in useChatSessionState keep fetching pages while the pane is unscrollable, each fill page re-pins via pendingFillPinRef; at a 2400px viewport a 60-tiny-row session auto-fetched 20→40 rows, scrollable, distFromBottom 0; fresh open, hard refresh, phone 390x844, and both panes with the worker pane open all land pinned with the last message above the composer.)*
- [x] The "Showing X of Y messages" banner and its scroll-up prompt are deleted; older messages load automatically as the user scrolls up (skeleton rows per phase 7 of ui11 while a page loads), silently. *(Verified: both banners, the load-all overlay, and their i18n keys across all 11 locales are gone — grep for showingOf/scrollToLoad/LoadAllMessagesOverlay returns nothing; scroll-to-top paged 21→41→61→81 rows with the banner regex never matching; the `older-page-skeleton` mounts during each fetch (MutationObserver-confirmed; resolves within a frame on localhost). Client-side windowing beyond 100 rows also expands silently at the top instead of the legacy "Showing last N" banner.)*
- [x] Reproduce Willem's case first (refresh a worker session mid-run with a long transcript), fix, and add a regression test for the fill-then-pin behavior. *(Reproduced on dev pre-fix with an 81-message synthetic mid-run session: "Showing 20 of 81 messages Scroll up to load more" on fresh open. Post-fix the same session opens pinned with no banner in both panes. Regression test: transcriptFill.test.ts, including a convergence case that fetches page-by-page until scrollable and asserts the pinned end state; test:client 81/81 pass.)*
