...
/**
 * Installs the versioned local server runtime used by Command Center Desktop.
 *
 * Server bundles are cached under:
 *   <runtime data directory>/server/<version>/dist-server/server/index.js
 */

const DEFAULT_INSTALL_ROOT = path.join(getLegacyDataDirectory(os.homedir()), 'server');
...
export class ServerInstaller {
  constructor({
    ...
    arch = process.arch,
    installRoot = readRenamedEnvironmentVariable('SERVER_DIR') || DEFAULT_INSTALL_ROOT,
    bundleBaseUrl = readRenamedEnvironmentVariable('SERVER_BUNDLE_URL') || DEFAULT_BUNDLE_BASE_URL,
    bundleReleaseTag = readRenamedEnvironmentVariable('SERVER_BUNDLE_RELEASE_TAG') || '',
    onLog,
  } = {}) {
  ...
  getBundleName() {
    return `command-center-local-server-${this.version}-${this.platform}-${this.arch}.tar.gz`;
  }
